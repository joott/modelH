...
Π .= load("tmpdata/thermalized_$ID"*"_$L"*".jld2")["Π"]
ϕ .= load("tmpdata/thermalized_$ID"*"_$L"*".jld2")["ϕ"]
...
[@time prethermalize(ϕ, Π, m², L^3) for i in 1:L]
  ...
  try
   mkdir("testdata_$oneovereta")
  catch 
   print("\n *The directory exists*\n")
  end 
  open("testdata_$oneovereta"*"/output_"*"$ID"*"_$L"*".dat","w") do io 
  open("testdata_$oneovereta"*"/outputpi_"*"$ID"*"_$L.dat","w") do iopi 
	  for i in 0:maxt
